...
from view_mode import get_view, mode_banner, adapt_title, adapt_caption, is_business, is_technical
import ifcopenshell
import json
import datetime
import uuid
...
def make_snapshot_png(topic, ifc_model=None):
    ...
    draw.text((12, 10), "IFC Semantic Data-Loss Analyser  ·  BCF Issue Snapshot", font=fnt_md, fill=(255, 255, 255))
    ...
    meta = (
        ...
        f"Author: {topic.get('author','IFC Semantic Data-Loss Analyser')}  |  "
        f"Date: {(topic.get('creation_date') or '')[:10]}"
    )
    draw.text((12, H - 38), meta,                                       font=fnt_sm, fill=(200, 200, 200))
    draw.text((12, H - 20), "Auto-generated by IFC Semantic Data-Loss Analyser  ·  BCF 2.1",  font=fnt_sm, fill=(140, 140, 140))
    ...
    if not rendered:
        ...
        fields = [
            ...
            ("Author",     topic.get("author",       "IFC Semantic Data-Loss Analyser")),
...
def auto_generate_topics(an, model, author, project_id, include_proxies,
    ...
    """Build BCF topic dicts from existing IFC Semantic Data-Loss Analyser analysis results."""
        ...
        for p in proxy_list[:max_topics]:
            topics.append({
                ...
                "description":      f"Element '{p['Name']}' (GlobalId: {p['GlobalId']}) is classified as IfcBuildingElementProxy. "
                                    ...
                                    f"Use Correction Suggestions in IFC Semantic Data-Loss Analyser to assign the correct IFC type.",
                "author":           author,
                "assigned_to":      "",
                "creation_date":    now,
                "modified_date":    now,
                "stage":            "Design",
                "labels":           ["ProxyElement", "SemanticLoss", "IFC Semantic Data-Loss Analyser"],
                "comments":         [{"text": "Auto-generated by IFC Semantic Data-Loss Analyser Proxy Detection Engine.", "author": "IFC Semantic Data-Loss Analyser", "date": now}],
    ...
    if include_psets:
        ...
        for p in pset_list[:max_topics]:
            topics.append({
                ...
                "modified_date":    now,
                "stage":            "Design",
                "labels":           ["MissingPset", "PropertyData", "IFC Semantic Data-Loss Analyser"],
                "comments":         [{"text": "Auto-generated by IFC Semantic Data-Loss Analyser Pset Analysis Engine.", "author": "IFC Semantic Data-Loss Analyser", "date": now}],
    ...
    if include_geo:
        ...
        for p in geo_list[:max_topics]:
            topics.append({
                ...
                "modified_date":    now,
                "stage":            "Design",
                "labels":           ["MissingGeometry", "GeometryLoss", "IFC Semantic Data-Loss Analyser"],
                "comments":         [{"text": "Auto-generated by IFC Semantic Data-Loss Analyser 5-Level Data Loss Analysis.", "author": "IFC Semantic Data-Loss Analyser", "date": now}],
                "component_guids":  [p.get("GlobalId","")] if p.get("GlobalId") else [],
                "related_guids":    [],
                "ifc_project_guid": project_id,
                "cam_x": 0, "cam_y": -10, "cam_z": 8,
    ...
    if include_rel:
        ...
        for p in rel_list[:max_topics]:
            topics.append({
                ...
                "modified_date":    now,
                "stage":            "Design",
                "labels":           ["MissingStoreyAssignment", "RelationshipLoss", "IFC Semantic Data-Loss Analyser"],
                "comments":         [{"text": "Auto-generated by IFC Semantic Data-Loss Analyser Relationship Loss Analysis.", "author": "IFC Semantic Data-Loss Analyser", "date": now}],
                "component_guids":  [p.get("GlobalId","")] if p.get("GlobalId") else [],
                "related_guids":    [],
                "ifc_project_guid": project_id,
                "cam_x": 0, "cam_y": -10, "cam_z": 8,
...
# ══════════════════════════════════════════════════════════════════════════════
with tab1:
    st.subheader("Auto-Generate BCF from IFC Semantic Data-Loss Analyser Analysis")
    ...
    with col1:
        ...
        ag_project_name = st.text_input("Project Name", value="IFC Semantic Data-Loss Analyser Project", key="ag_proj")
        ag_author       = st.text_input("Author / Coordinator", value=st.session_state.get("user_context",{}).get("name","IFC Semantic Data-Loss Analyser User"), key="ag_author")
    ...
    if preview_count == 0:
        ...
        if st.button("⚡ Generate BCF File", use_container_width=True, key="ag_gen"):
            with st.spinner(f"Building {preview_count} BCF topics..."):
                ...
                st.session_state["bcf_filename"] = f"IFC Semantic Data-Loss Analyser_{ag_project_name.replace(' ','_')}.bcfzip"
...
with tab2:
    ...
    with st.expander("➕ Add New Issue", expanded=len(st.session_state.manual_topics)==0):
        ...
        m_author   = fd3.text_input("Author", value=st.session_state.get("user_context",{}).get("name","IFC Semantic Data-Loss Analyser User"), key="m_auth")
        ...
        if st.button("➕ Add Issue to List", key="m_add"):
            if not m_title.strip():
                ...
                st.session_state.manual_topics.append({
                    ...
                    "modified_date":    now,
                    "stage":            m_stage,
                    "labels":           labels if labels else ["IFC Semantic Data-Loss Analyser"],
                    "comments":         comments,
    ...
    if st.session_state.manual_topics:
        ...
        man_author  = mc2.text_input("Author",
                      value=st.session_state.get("user_context",{}).get("name","IFC Semantic Data-Loss Analyser User"),
                      key="man_auth_exp")
...
BIMcollab, Tekla Structures, and over 100 other BIM tools.
    ...
    st.code("""IFC Semantic Data-Loss Analyser_Project.bcfzip
