...
from view_mode import get_view, mode_banner, adapt_title, adapt_caption, is_business
import ifcopenshell
import pandas as pd
...
with eng_tab1:
    if not proxy_corrections:
        ...
        with col_info_aa:
            ...
            if removed_count:
                removed_html = f'  ·  <strong style="color:#da3633;">{removed_count}</strong> removed'
            else:
                removed_html = ''
            st.markdown(
                f"<div style='padding:8px 14px;background:#161b22;border:1px solid #30363d;"
                f"border-radius:8px;font-size:13px;color:#8b949e;'>"
