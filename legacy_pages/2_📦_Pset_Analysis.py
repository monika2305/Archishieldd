...
st.title(adapt_title("📦 Property Set (Pset) Analysis", "📦 Missing Data — Property Completeness"))
st.caption(adapt_caption("Completeness check for standard IFC property sets across all element types.", "How many elements are missing the data needed for costing, scheduling, and compliance."))
st.markdown("---")
...
with tab1:
    st.subheader(adapt_title("Pset Completeness by IFC Type", "Missing Data by Element Category"))
...
with tab2:
    st.subheader(adapt_title("Element-level Pset Details", "Element-level Missing Data"))
